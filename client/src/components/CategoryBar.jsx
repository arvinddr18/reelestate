...
export default function CategoryBar({ onFilterChange, activeCategory, activeSub, onSubSelect, onReelClick }) {
  ...
  return (
    ...
    <div className="w-full bg-[#0B0F19] flex flex-col border-b border-white/5">
      ...
      {/* ── ROW 2: CATEGORY TEXT PILLS (Glassy Look) ── */}
      <div className="w-full pt-1 pb-4 flex items-center">
            ...
            return (
              <button 
                ...
                className={`px-5 py-2.5 rounded-full text-[13px] font-bold transition-all duration-300 backdrop-blur-md active:scale-95 ${
                  ...
                  : 'bg-[#151A25]/80 border border-white/5 text-gray-400 hover:text-white hover:bg-white/10'
                }`}
              >
                {/* Only text, no icons! */}
                {cat.name}
      ...
      <div className="flex items-center relative w-full pt-2 pb-6 border-t border-white/5">
        ...
        {/* CENTER: SCROLLABLE USERS STORIES */}
