...
export default function Messages() {
  ...
  useEffect(() => {
    const fetchNetwork = async () => {
      ...
      try {
        ...
        } catch (err) { console.error("🔴 Following Fetch Error:", err.message); }
        ...
        } catch (err) { console.error("🔴 Followers Fetch Error:", err.message); }
  ...
  return (
    <>
      {/* GLUED TO SCREEN: Prevents the white bar glitch at the bottom */}
      <div className="fixed top-0 left-0 w-full h-full bg-[#05070A] text-white font-sans flex overflow-hidden">
        ...
        <div className={`w-full md:w-[400px] lg:w-[450px] h-full flex flex-col bg-black/20 backdrop-blur-md border-r border-[#1E2532] z-10 shrink-0 transition-transform duration-500 ${activeChat ? 'hidden md:flex' : 'flex'}`}>
          <header className="px-6 py-5 shrink-0 relative z-10">
            ...
            <div className="bg-white/5 border border-white/10 rounded-full p-1.5 flex items-center shadow-inner focus-within:border-[#00F0FF]/50 transition-colors backdrop-blur-xl">
              ...
              <input 
                type="text" placeholder="Search your network..." value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)}
        ...
        {/* 🚨 DYNAMIC INLINE STYLE: This physically shrinks the chat window when the keyboard opens */}
        <div 
          className={`${!activeChat ? 'hidden md:flex flex-1 relative items-center justify-center pointer-events-none' : 'flex flex-col absolute top-0 left-0 w-full z-[9999] bg-[#05070A] md:relative md:flex-1 md:bg-transparent'}`}
          style={activeChat ? { height: vpHeight } : {}}
        >
          ...
          {activeChat && (
            <>
              {/* 1. FLEX HEADER - shrink-0 ensures it never gets crushed */}
              ...
              <div className="shrink-0 relative w-full bg-[#05070A]/98 backdrop-blur-2xl border-t border-white/10 pt-2 pb-2 md:pb-6 px-2 md:px-8 z-20 flex flex-col items-center gap-2">
                ...
                <div className="relative w-full max-w-3xl flex items-center gap-1 md:gap-2 bg-[#1A1F2E]/90 backdrop-blur-2xl border border-white/20 p-1 md:p-1.5 rounded-full shadow-[0_15px_40px_rgba(0,0,0,0.8)] focus-within:border-[#bc00dd]/50 focus-within:shadow-[0_0_30px_rgba(188,0,221,0.2)] transition-all duration-300">
                  ...
                  {/* 🚨 text-[16px] prevents iOS/Android auto-zoom bug */}
