...
// 👇 FIXED: Added IoMdText to the import list so it doesn't crash!
...
const VideoNode = ({ src, isActive }) => {
  ...
  return (
    <video ref={videoRef} src={src} loop muted playsInline className="absolute inset-0 w-full h-full object-cover" />
  );
};
  ...
  return (
    <div className="h-screen w-full bg-[#05070A] relative overflow-hidden flex justify-center">
      ...
      <div className="absolute top-0 left-0 right-0 z-50 p-4 md:p-8 flex items-center justify-between pointer-events-none">
        <button onClick={() => navigate(-1)} className="w-12 h-12 rounded-full bg-[#0B0F19]/80 backdrop-blur-xl border border-white/10 flex items-center justify-center text-white hover:text-[#00F0FF] transition-all shadow-lg pointer-events-auto">
          <IoMdArrowBack size={24} />
        </button>
      ...
      <div onScroll={handleScroll} className="w-full max-w-[450px] h-screen overflow-y-scroll snap-y snap-mandatory no-scrollbar relative z-10 md:py-10">
        {posts.map((post, index) => {
          ...
          return (
            <div key={post._id || index} className="w-full h-screen md:h-[calc(100vh-80px)] snap-start snap-always relative flex items-center justify-center md:px-4">
                  ...
                  <div className={`transition-all duration-700 ${isActive ? 'translate-y-0 opacity-100' : 'translate-y-10 opacity-0'}`}>
                    ...
                    <div className="flex items-center gap-3 mb-4 w-max cursor-pointer">
                      <div className="relative w-10 h-10 rounded-full bg-gradient-to-tr from-[#0057FF] to-[#00F0FF] p-[2px] shadow-[0_0_15px_rgba(0,240,255,0.3)]">
                        <img src={resolveMediaUrl(post.author?.profilePhoto) || `https://ui-avatars.com/api/?name=${post.author?.username || 'Node'}&background=0B0F19&color=00F0FF`} alt="avatar" className="w-full h-full rounded-full object-cover border border-[#0B0F19]" />
                      </div>
                      <div className="flex flex-col">
                        <span className="text-white font-black text-sm drop-shadow-md flex items-center gap-2">@{post.author?.username || 'user'}</span>
                      </div>
                    </div>
                    ...
                    <div className="bg-[#151A25]/80 backdrop-blur-md border border-white/10 p-4 rounded-2xl relative overflow-hidden shadow-xl">
                      ...
                      {post.location && (
                        <p className="flex items-center gap-1.5 text-gray-300 text-xs font-bold mb-3"><IoMdPin className="text-[#00F0FF]" /> {post.location}</p>
                      )}
                ...
                <div className="absolute bottom-6 md:bottom-10 right-2 md:right-4 z-20 pointer-events-auto">
                  <div className={`flex flex-col gap-4 bg-[#0B0F19]/60 backdrop-blur-xl border border-white/10 p-2 md:p-3 rounded-full transition-all duration-700 ${isActive ? 'translate-x-0 opacity-100' : 'translate-x-10 opacity-0'}`}>
                    ...
                    <button className="flex flex-col items-center gap-1 group active:scale-95 transition-transform">
                      <div className="w-10 h-10 rounded-full bg-[#151A25] flex items-center justify-center text-white group-hover:text-red-500 transition-colors"><IoMdHeart size={20} /></div>
                      <span className="text-[9px] font-black text-white">{post.likesCount || '0'}</span>
                    </button>

                    <button className="flex flex-col items-center gap-1 group active:scale-95 transition-transform">
                      <div className="w-10 h-10 rounded-full bg-[#151A25] flex items-center justify-center text-white group-hover:text-[#00F0FF] transition-colors">
                        <IoMdText size={20} /> 
                      </div>
                    ...
                    <button className="flex flex-col items-center gap-1 group active:scale-95 transition-transform">
                      <div className="w-10 h-10 rounded-full bg-[#151A25] flex items-center justify-center text-white group-hover:text-yellow-400 transition-colors"><IoMdBookmark size={20} /></div>
                    ...
                    <button className="flex flex-col items-center gap-1 mt-2 group active:scale-95 transition-transform">
                      <div className="w-10 h-10 rounded-full bg-gradient-to-tr from-[#0057FF] to-[#00F0FF] flex items-center justify-center text-white shadow-[0_0_15px_rgba(0,240,255,0.4)]"><IoMdShareAlt size={20} /></div>
                    </button>
