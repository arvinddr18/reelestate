...
export default function ScrollPage() {
  ...
  useEffect(() => {
    const fetchPosts = async () => {
      try {
        ...
        // Extremely strict array checking
        let fetchedData = [];
  ...
  if (!posts || posts.length === 0) {
    return (
      <div className="h-screen w-full bg-[#05070A] flex flex-col items-center justify-center p-6 text-center relative">
        ...
        <div className="w-24 h-24 rounded-full bg-[#151A25] border border-[#1E2532] flex items-center justify-center text-4xl mb-6 shadow-lg">
          🎬
        </div>
  ...
  // 3. SAFE MAIN RENDER VARIABLES
  const activePost = posts[activeIndex] || posts[0];
  const activeMediaUrl = activePost ? resolveMediaUrl(activePost.images?.[0]?.url || activePost.image) : '';
